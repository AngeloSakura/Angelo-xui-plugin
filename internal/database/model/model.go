...
type Inbound struct {
	...
	// TrafficMultiplier scales client/inbound traffic counters when Xray stats
	// are committed to the DB. 1.0 = no scaling (default, bit-exact legacy
	// behaviour). Applied per-inbound so a single panel can host inbounds with
	// different cost ratios (e.g. a 5x-marked entry for premium peering). The
	// multiplier is multiplied into Xray's reported deltas at write time, so
	// every downstream consumer (quota check, subscription, node sync, master
	// panel) sees the scaled number without needing to know the rule exists.
	// Clamped to [0.1, 100] on write so an operator typo cannot zero or
	// invert traffic; out-of-range values fall back to 1.0 with a warning.
	TrafficMultiplier float64 `json:"trafficMultiplier" form:"trafficMultiplier" gorm:"column:traffic_multiplier;default:1.0" validate:"omitempty,gte=0.1,lte=100" example:"1.0"`
...
// TrafficMultiplier bounds used at every read/write so a UI typo or stale row
// cannot zero, invert or overflow the per-client traffic counter. Values outside
// the range fall back to 1.0 (no scaling) at the call site via NormalizeTrafficMultiplier.
const (
	trafficMultiplierMin = 0.1
	trafficMultiplierMax = 100.0
)

// NormalizeTrafficMultiplier returns the multiplier with out-of-range and
// non-finite inputs clamped to 1.0. The traffic path applies it to every Xray
// delta, so a stray 0 here would silently block every client on the inbound.
//
// The order of checks matters: NaN/Inf compare false to every bound, so we
// must test for finiteness FIRST. Otherwise a NaN row would round-trip through
// the traffic loop as-is and silently produce out-of-band values.
func NormalizeTrafficMultiplier(m float64) float64 {
	if isNaNOrInf(m) {
		return 1.0
	}
	if m < trafficMultiplierMin || m > trafficMultiplierMax {
		return 1.0
	}
	return m
}

// isNaNOrInf covers the two non-finite cases that must be treated as
// out-of-range. math.IsNaN and math.IsInf exist, but importing math just
// for these is overkill — they are one-comparison primitives. The float
// comparison `f != f` is the canonical NaN check; |f| > MaxFloat64 catches
// ±Inf (and the rare denormal-as-Inf that some encoders emit).
func isNaNOrInf(f float64) bool {
	return f != f || f > math.MaxFloat64/2 || f < -math.MaxFloat64/2
}

// ScaledTrafficBytes multiplies raw Xray-reported bytes by the multiplier and
// rounds to int64. Negative deltas (Xray emits none today, but the stats API
// contract permits them) are passed through unchanged so a future client reset
// path can subtract rather than accidentally negate the multiplier.
func ScaledTrafficBytes(raw int64, multiplier float64) int64 {
	if raw == 0 {
		return 0
	}
	if raw < 0 {
