...
func (s *InboundService) addInboundTraffic(tx *gorm.DB, traffics []*xray.Traffic) error {
	...
	// Load multipliers for every local inbound in one query so the inner loop is
	// allocation-free. Untagged entries (e.g. legacy stats rows) still match the
	// existing tag=WHERE clause and fall through with multiplier 1.0.
	tagMultipliers, err := loadInboundTrafficMultipliersByTag(tx)
	if err != nil {
		return err
	}
	...
	for _, traffic := range traffics {
		if traffic.IsInbound {
			multiplier := tagMultipliers[traffic.Tag]
			// Multiply Xray's reported deltas before they enter the DB so every
			// downstream reader (quota check, subscription, node sync, master
			// panel) sees the scaled number and does not need to know the rule
			// exists. ScaledTrafficBytes clamps 0/negative/out-of-range inputs.
			upDelta := model.ScaledTrafficBytes(traffic.Up, multiplier)
			downDelta := model.ScaledTrafficBytes(traffic.Down, multiplier)
			err = tx.Model(&model.Inbound{}).Where("tag = ? AND node_id IS NULL", traffic.Tag).
				Updates(map[string]any{
					"up":   gorm.Expr(database.ClampedAddExpr("up"), upDelta),
					"down": gorm.Expr(database.ClampedAddExpr("down"), downDelta),
				}).Error
			if err != nil {
				return err
			}
...
// loadInboundTrafficMultipliersByTag returns every local (node_id IS NULL)
// inbound's traffic_multiplier keyed by tag. Multipliers outside [0.1, 100] or
// stored as NaN/Inf fall back to 1.0 via NormalizeTrafficMultiplier so a
// corrupt row cannot zero or invert the panel's traffic counters. Empty result
// means no inbounds exist yet — every caller treats a missing key as 1.0.
func loadInboundTrafficMultipliersByTag(tx *gorm.DB) (map[string]float64, error) {
	type row struct {
		Tag               string
	...
	for _, r := range rows {
		...
		out[r.Tag] = model.NormalizeTrafficMultiplier(r.TrafficMultiplier)
	}
	return out, nil
}

// loadInboundTrafficMultipliersByID is the client-traffic twin of
// loadInboundTrafficMultipliersByTag: keyed by inbound id so the client traffic
// loop can pick a multiplier from client_traffics.inbound_id in O(1).
func loadInboundTrafficMultipliersByID(tx *gorm.DB) (map[int]float64, error) {
	type row struct {
		Id                int
		TrafficMultiplier float64
	}
	...
	for _, r := range rows {
		out[r.Id] = model.NormalizeTrafficMultiplier(r.TrafficMultiplier)
	}
	return out, nil
}
...
func (s *InboundService) addClientTraffic(tx *gorm.DB, traffics []*xray.ClientTraffic) (err error) {
	...
	// Load per-inbound multipliers once for the whole batch. client_traffics is
	// email-keyed (one row per email regardless of how many inbounds), and each
	// row carries an inbound_id picked up at insert time — usually the inbound
	// the client was first attached to. Multiplying by that inbound's multiplier
	// matches the existing "attribute traffic to one inbound" pattern; for a
	// client attached to two inbounds with different multipliers the
	// first-attached inbound wins, which is the same ambiguity the rest of the
	// traffic pipeline already has. A missing inbound id falls back to 1.0.
	idMultipliers, err := loadInboundTrafficMultipliersByID(tx)
	if err != nil {
		return err
	}
	...
	for _, ct := range dbClientTraffics {
		...
		multiplier := idMultipliers[ct.InboundId]
		scaledUp := model.ScaledTrafficBytes(t.Up, multiplier)
		scaledDown := model.ScaledTrafficBytes(t.Down, multiplier)
		if err = tx.Exec(
			fmt.Sprintf(
