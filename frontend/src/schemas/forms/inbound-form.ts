...
export const InboundDbFieldsSchema = z.object({
  ...
  // Scales Xray-reported traffic deltas before they hit client_traffics. 1.0 =
  // no scaling (legacy behaviour). Backend enforces [0.1, 100] in NormalizeTrafficMultiplier,
  // so a value outside that range here is silently clamped on the next save.
  trafficMultiplier: z.number().min(0.1).max(100).default(1),
});
