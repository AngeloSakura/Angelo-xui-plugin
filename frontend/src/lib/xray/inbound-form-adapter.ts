...
export interface RawInboundRow {
  ...
  subSortIndex?: number;
  disableFlow?: boolean;
  // Optional because inbound rows written before the feature was added lack it;
  // the adapter falls back to 1.0 so legacy rows always default to identity.
...
export interface WireInboundPayload {
  ...
  subSortIndex: number;
  disableFlow: boolean;
  // Defaults to 1 on the wire so an older panel that doesn't know about the
  // field still receives a valid number when a row was just loaded.
  trafficMultiplier: number;
}
...
// Mirrors backend model.NormalizeTrafficMultiplier: anything outside the
// supported [0.1, 100] range (or a missing value from a legacy row) reads back
// as 1.0 so the input shows the same number the backend will apply.
function normalizeMultiplierOnRead(v: unknown): number {
  if (typeof v !== 'number' || !Number.isFinite(v)) return 1;
  if (v < 0.1 || v > 100) return 1;
  return v;
}
...
export function rawInboundToFormValues(row: RawInboundRow): InboundFormValues {
  ...
  return {
    ...
    // Clamp here too: a row written by an older panel leaves the field
    // undefined; the backend's NormalizeTrafficMultiplier would also clamp
    // but doing it on read keeps the input box in a sane state.
    trafficMultiplier: normalizeMultiplierOnRead(row.trafficMultiplier),
    protocol,
    settings,
...
export function formValuesToWirePayload(values: InboundFormValues): WireInboundPayload {
  ...
  const payload: WireInboundPayload = {
    ...
    // Send the typed field through; an explicit 0 in the input would round-
    // trip to 1.0 at the backend (NormalizeTrafficMultiplier), so the user
    // sees the persisted 1.0 next time the modal re-opens.
